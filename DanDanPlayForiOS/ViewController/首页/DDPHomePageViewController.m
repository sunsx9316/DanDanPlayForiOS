...
- (void)viewDidLoad {
    ...
    [[DDPCacheManager shareCacheManager] addObserver:self forKeyPath:@"user" options:NSKeyValueObservingOptionNew context:nil];
...
- (void)dealloc {
    ...
    [[DDPCacheManager shareCacheManager] removeObserver:self forKeyPath:@"user"];
}
    ...
    if (_tableView == nil) {
        ...
        _tableView.mj_header = [MJRefreshNormalHeader ddp_headerRefreshingCompletionHandler:^{
            ...
            if ([DDPCacheManager shareCacheManager].user) {
                [DDPPlayHistoryNetManagerOperation playHistoryWithUser:[DDPCacheManager shareCacheManager].user completionHandler:^(DDPBangumiQueueIntroCollection *responseObject, NSError *error) {
                    if (error) {
                        [self.view showWithError:error];
                    }
                    else {
