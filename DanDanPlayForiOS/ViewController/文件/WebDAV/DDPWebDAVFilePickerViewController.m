//
//  DDPWebDAVFilePickerViewController.m
//  DDPlay
//
...
@interface DDPWebDAVFilePickerViewController ()<UITableViewDelegate, UITableViewDataSource>
@property (strong, nonatomic) DDPBaseTableView *tableView;
@property (strong, nonatomic) UIImage *folderImg;
@end

@implementation DDPWebDAVFilePickerViewController
...
- (DDPBaseTableView *)tableView {
    if (_tableView == nil) {
        ...
        _tableView.mj_header = [MJRefreshHeader ddp_headerRefreshingCompletionHandler:^{
            @strongify(self)
            if (!self) return;
            [[DDPToolsManager shareToolsManager] startDiscovererWebDevFileWithParentFile:(DDPWebDAVFile *)self.file fileType:self.fileType completion:^(DDPWebDAVFile *aFile, NSError *error) {
                if (error) {
