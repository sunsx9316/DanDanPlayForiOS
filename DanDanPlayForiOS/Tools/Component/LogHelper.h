...
typedef NS_ENUM(NSUInteger, DDPLogLevel) {
    DDPLogLevelAll = 0,
    DDPLogLevelVerbose = 0,
    DDPLogLevelDebug,    // Detailed information on the flow through the system.
    DDPLogLevelInfo,     // Interesting runtime events (startup/shutdown), should be conservative and keep to a minimum.
    DDPLogLevelWarn,     // Other runtime situations that are undesirable or unexpected, but not necessarily "wrong".
    DDPLogLevelError,    // Other runtime errors or unexpected conditions.
    DDPLogLevelFatal,    // Severe errors that cause premature termination.
    DDPLogLevelNone,     // Special level used to disable all log messages.
};
