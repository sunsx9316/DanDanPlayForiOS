...
- (void)awakeFromNib {
    ...
    for (NSDictionary *dic in colorArr) {
        let item = [[DDPDanmakuColorMenuItem alloc] initWithTitle:dic[@"name"] color:[NSColor colorWithRGB:(uint32_t)[dic[@"value"] integerValue]]];
...
- (NSString *)playerSliderView:(DDPPlayerSlider *)sliderView didShowTipsAtProgress:(CGFloat)progress {
    ...
    let currentTimeStr = ddp_mediaFormatterTime(currentTime);
    let totalTimeStr = ddp_mediaFormatterTime(self.totalTime);
